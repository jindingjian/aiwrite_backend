...
public class UserController {
    ...
    public Result<BigDecimal> getBalance(@Parameter(description = "用户ID") @PathVariable Long userId) {
        // 查询余额
    ...
    public Result<List<UserForm>> getUser(
            @Parameter(description = "用户ID") @PathVariable Long userId
    ) {
        List<UserForm> userForm = userService.getParentIdByUserId(userId);
        return Result.success(userForm);
    }

    @Operation(summary = "付款")
    @PostMapping("/{userId}/updateBalance")
    public Result<?> updateBalance(@RequestParam BigDecimal balance,@Parameter(description = "用户ID") @PathVariable Long userId){
        Boolean b = userService.updateBalance(balance, userId);
